# Convolutional Neural Network classifier applied to music genre classification
...
def prepare_datasets(test_size, validation_size):
    ...
    # tf/CNN expect a 3d array for each sample (130,13,1) 130 is the number of time bin per samples with this dataset
...
def build_model(input_shape):
    ...
    #4flatten the output and feed it into dense layer - fully connected layer for classification
    model.add(keras.layers.Flatten())
    model.add(keras.layers.Dense( 64, activation = 'relu')) #fed flat array into a 64 neuron dense layer
    model.add(keras.layers.Dropout(0.3))  # 30% neuron dropout probability to takle overfitting
...
if __name__ == "__main__":
    ...
    # 2 cuild the CNN net
    input_shape = (X_train.shape[1],X_train.shape[2],X_train.shape[3]) # 3d array! X_train.shape[0] is the sample bin
